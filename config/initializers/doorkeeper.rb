  ...
  Doorkeeper.configure do
    ...
    resource_owner_authenticator do
      if current_user
        ...
        warden.authenticate!(scope: :fc_user)
        nil
      end
    end
    ...
    hash_application_secrets using: '::Oauth::SecretStoring::Plain'
    ...
    # Doorkeeperは本来JWTをサポートしていないので、モンキーパッチして対応できるようにしている。
    # 該当のソースコードは以下を参照。
    # app/models/oauth/application.rb
    # lib/monkey_patches/doorkeeper/server_patch.rb
    # lib/monkey_patches/doorkeeper/oauth/client/credentials_patch.rb

    client_credentials :from_jwt
    ...
    after_successful_authorization do |_controller, context|
      next unless context.auth.is_a?(Doorkeeper::OAuth::CodeResponse)

      context.auth.auth.token.save_commmune_login_datetime
    end
