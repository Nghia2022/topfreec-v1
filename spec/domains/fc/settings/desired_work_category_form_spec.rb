...
RSpec.describe Fc::Settings::DesiredWorkCategoryForm, type: :model do
      ...
      describe 'length' do
        it do
          is_expected.to allow_value(WorkCategory.pluck(:sub_category).flatten.sample(100)).for(:desired_works_sub)
            .and not_allow_value(WorkCategory.pluck(:sub_category).flatten.sample(101)).for(:desired_works_sub).with_message('選択できる領域数は100件を上限としております。')
        end
      end
    end
    ...
    describe '#save' do
      ...
      context 'with valid' do
        it do
          expect do
            form.save(person)
          end.to change { contact.reload.desired_works_sub__c }.to(desired_works_sub)
            .and change { contact.reload.desired_works_main__c }.to(desired_works_main)
        end
      end
      ...
      context 'with invalid' do
        let(:params) do
          {
            desired_works_sub: desired_works_sub.push('テスト')
          }
        end
        ...
        it do
          expect do
            form.save(person)
          end.to not_change(contact, :desired_works_sub__c)
            .and not_change(contact, :desired_works_main__c)
        end

        it 'changed attribute of desired_works_main' do
          form.save(person)
          expect(form.desired_works_main).to eq(desired_works_main)
        end
      end
    end
  end
