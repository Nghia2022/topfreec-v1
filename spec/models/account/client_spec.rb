...
RSpec.describe Account::Client, type: :model do
  it_behaves_like 'sobject', 'Account'

  describe 'associations' do
    it { is_expected.to have_many(:client_users).with_foreign_key(:account_sfid).with_primary_key(:sfid) }
    it { is_expected.to have_many(:contacts).with_foreign_key(:accountid).with_primary_key(:sfid) }
    it { is_expected.to have_many(:projects).with_foreign_key(:accountid).with_primary_key(:sfid) }
