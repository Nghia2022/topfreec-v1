...
RSpec.describe Content::ColumnCell, type: :cell do
  controller ApplicationController

  let(:options) { {} }
  let(:described_cell) { cell(described_class, model, options) }
  let(:model) { FactoryBot.build_stubbed(:column).decorate }
  let(:term) { FactoryBot.build(:wp_term) }

  before do
    allow_any_instance_of(Wordpress::Column).to receive(:terms).and_return([term])
  ...
  context 'cell rendering' do
    describe 'rendering #show' do
      ...
      it do
        is_expected.to have_content(model.post_title)
          .and have_link(href: content_column_path(model))
      end
    end
    ...
    describe 'rendering #recommended' do
      ...
      it { is_expected.to have_content('【SAPコンサルタント必読】SAP4HANAとERPの違いや導入メリットと将来性を整理」') }
    end
  end
  ...
  describe '#detail_path' do
    ...
    it do
      is_expected.to eq(content_column_path(model))
        .and ending_with('/')
    end
  end
