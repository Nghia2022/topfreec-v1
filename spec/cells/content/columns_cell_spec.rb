...
RSpec.describe Content::ColumnsCell, type: :cell do
  controller ApplicationController

  let(:options) { {} }
  let(:described_cell) { cell(described_class, model, options) }
  let(:model) { double('Columns') }
  let(:columns) { FactoryBot.build_stubbed_list(:column, 2) }
  ...
  context 'cell rendering' do
    ...
    describe 'rendering #show' do
      ...
      before do
        ...
        allow(model).to receive(:decorate).and_return(Wordpress::ColumnDecorator.decorate_collection(columns))
      end
