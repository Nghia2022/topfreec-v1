  ...
  module Routing
    class MarkdownFormatter
      ...
      def draw_header(routes)
        ...
        divider = "| #{'-' * name_width} | #{'-' * [verb_width, 4].max} | #{'-' * path_width} | #{'-' * reqs_width} |"
        header + divider
      end
...
namespace :routes do
  desc 'Print out all defined routes with markdown style'
  task markdown: %i[environment] do
    all_routes = Rails.application.routes.routes
    require 'action_dispatch/routing/inspector'
    inspector = ActionDispatch::Routing::RoutesInspector.new(all_routes)
    puts inspector.format(ActionDispatch::Routing::MarkdownFormatter.new, controller: ENV.fetch('CONTROLLER', nil))
  end
end
