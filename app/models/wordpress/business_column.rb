...
class Wordpress::BusinessColumn < Wordpress::WpPost
  ...
  def prev_content
    @prev_content ||= Wordpress::BusinessColumn.latest_order.find_by('post_date < ?', post_date)
  end

  def next_content
    @next_content ||= Wordpress::BusinessColumn.latest_order.where('post_date > ?', post_date).last
  end
  ...
  class << self
    ...
    def model_name
      ActiveModel::Name.new(self, nil, 'corp_business_column')
    end

    def decorator_class
      Wordpress::BusinessColumnDecorator
    end
  end
end
