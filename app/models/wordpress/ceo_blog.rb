...
class Wordpress::CeoBlog < Wordpress::WpPost
  scope :latest_order, -> { order(post_date: :desc) }

  def self.policy_class
    Content::CeoBlogPolicy
  end

  def prev_content
    @prev_content ||= Wordpress::CeoBlog.published.latest_order.find_by('post_date < ?', post_date)
  end

  def next_content
    @next_content ||= Wordpress::CeoBlog.published.latest_order.where('post_date > ?', post_date).last
  end
  ...
  class << self
    def model_name
      ActiveModel::Name.new(self, nil, 'content_blog')
    end

    def decorator_class
      Wordpress::CeoBlogDecorator
    end
  end
end
